...
# Step 3: Define the Simple RNN Model
class SimpleRNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, dropout):
        super(SimpleRNN, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=vocab['<PAD>'])
        self.rnn = nn.RNN(embedding_dim, hidden_dim, num_layers=n_layers,
                          dropout=dropout, batch_first=True)  # Simple RNN
    ...
    def forward(self, text, text_lengths):
        ...
        hidden = self.dropout(hidden[-1])  # Last hidden state of the RNN
...
model = SimpleRNN(len(vocab), embedding_dim, hidden_dim, output_dim, n_layers, dropout)

# Step 4: Train the Simple RNN Model
